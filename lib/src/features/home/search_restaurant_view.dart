...
class SearchRestaurantView extends ConsumerStatefulWidget {
  ...
  const SearchRestaurantView({Key? key, required this.initialQuery}) : super(key: key);
...
class _SearchRestaurantViewState extends ConsumerState<SearchRestaurantView> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredRestaurants = restaurants.asMap().entries.where((entry) => entry.value.name.toLowerCase().contains(query.toLowerCase()))
        .map((entry) => MapEntry(entry.key, entry.value)).toList();
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: Row(
                  ...
                  children: [
                    if (query.isNotEmpty)
                      IconButton(
                        icon: const Icon(Iconsax.close_circle, color: kcPrimaryRed200, size: 20),
                        onPressed: () {
          ...
          Expanded(
            child: filteredRestaurants.isEmpty
              ...
              itemBuilder: (context, index) {
                ...
                final originalIndex = entry.key; // Original index in restaurantProvider
